...
shared_ptr<Node> ParseExpression(It& current, It end, unsigned precedence)
{
	...
	while (current != end && current->type != TokenType::PAREN_RIGHT) 
	{
		...
		const auto logicalOperation = current->value == "AND" ? LogicalOperation::And : LogicalOperation::Or;
		...
		left = make_shared<LogicalOperationNode>(logicalOperation, left, ParseExpression(current, end, currentPrecedence));
	}
